  ...
  try {
    ...
    if (request.headers["content-type"]?.includes("multipart/form-data")) {
      ...
      if (request.query && (request.query as any).projet_id) {
        ...
        console.log(
          "[DEBUG] restrictToFirstUser: projet_id trouvé dans URL:",
          projet_id
        );
      }
      ...
      if (!projet_id && request.headers["x-projet-id"]) {
        ...
        console.log(
          "[DEBUG] restrictToFirstUser: projet_id trouvé dans header:",
          projet_id
        );
      }
    } else {
      ...
      console.log(
        "[DEBUG] restrictToFirstUser: projet_id depuis body:",
        projet_id
      );
    }
    ...
    console.log(
      "[DEBUG] restrictToFirstUser: Vérification pour userId:",
      userId,
      "projet_id:",
      projet_id
    );
    ...
    if (!result.rows.length) {
      console.log(
        "[DEBUG] restrictToFirstUser: Aucune étape 1 trouvée pour ce projet"
      );
    ...
    if (firstStepUserId !== userId) {
      console.warn("[DEBUG] restrictToFirstUser: Accès refusé", {
        userId,
        firstStepUserId,
        projet_id,
      });
      return reply
        .code(403)
        .send({
          error: "Seul l'utilisateur de l'étape 1 peut effectuer cette action",
        });
    }
